...
export async function startSiegeMonitor(client, guildId) {
  ...
  if (unifiedChat) {
    await unifiedChat.send("⚠️ **تنبيه:** سيتم إخلاء القنوات الصوتية تلقائياً للأعضاء غير الإداريين بعد مرور ساعة (60 دقيقة) من بداية السيج.\n⚠️ **Notice:** Voice channels will be automatically cleared for non-admins 1 hour (60 minutes) after the Siege starts.");
  }
    ...
    if (inVc) {
      ...
      if (record.everJoined && record.checks < REQUIRED_CHECKS && !record.withdrawnAnnounced) {
        ...
        if (elapsed >= 20 * 60 * 1000) {
          ...
          const unifiedChat = guild.channels.cache.get(UNIFIED_CHAT_ID) ?? await guild.channels.fetch(UNIFIED_CHAT_ID).catch(() => null);
          if (unifiedChat) {
            await unifiedChat.send(`⚠️ تم انسحاب العضو <@${userId}> (${record.character_name}) من السيج مبكراً! | Member has withdrawn from Siege early!`);
          }
        }
      }
    }
...
async function endSiege(client, guild) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    const embed = new EmbedBuilder()
      .setColor(0x2b2d31)
      .setTitle(i === 0 ? "📊 نتائج ومنافسة السيج (Siege Results & Leaderboard)" : "📊 تكملة نتائج السيج")
...
async function clearVoiceChannels(client, guild) {
  try {
    const unifiedChat = guild.channels.cache.get(UNIFIED_CHAT_ID) ?? await guild.channels.fetch(UNIFIED_CHAT_ID).catch(() => null);
    if (unifiedChat) {
      await unifiedChat.send("⚠️ **تنبيه:** سيتم الآن إنهاء الجلسات الصوتية وتفريغ الرومات للأعضاء. شكراً لجهودكم!\n⚠️ **Notice:** Voice sessions are now being terminated. Thank you for your efforts!");
    }
