...
async function handleCreateGroup(interaction, dungeonName, difficulty) {
  ...
  if (generalChannel) {
    ...
    const creationMsg = await generalChannel.send({
      content: `مجموعة ${creatorName} يبغون يدخلون دجن ${dungeonName} الان الي يبي يدخل يلحق يضغط على الزر💪`
    }).catch(() => null);
...
export async function handleLfgVoiceLeave(client, member, channel) {
  const res = await query("SELECT id FROM dungeon_lfg_groups WHERE voice_channel_id=$1 AND status != 'expired'", [channel.id]);
  if (!res.rows[0]) return;
  const groupId = res.rows[0].id;
...
export async function cleanUpEmptyLfg(client, groupId) {
  ...
  if (vc && vc.members.size === 0 && (hasHourPassed || isQuickFinish)) {
    console.log(`[LFG:Cleanup] Deleting empty room for group #${groupId}. HourPassed: ${hasHourPassed}, QuickFinish: ${isQuickFinish}`);
    await vc.delete().catch(() => {});
