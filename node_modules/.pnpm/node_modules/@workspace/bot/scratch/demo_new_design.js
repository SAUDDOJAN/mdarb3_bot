...
async function runDemo() {
  ...
  client.once("ready", async () => {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: "الأعضاء 👥",
          value: 
            `1️⃣ 👑 <@${leaderId}> **[Templar]** ✅ Ready\n` +
            `2️⃣ <@123456789> **[Ranger]** ⏳ Waiting\n` +
            `3️⃣ ▪️ **فارغ** (DPS)\n` +
            `4️⃣ ▪️ **فارغ** (Healers)`,
          inline: false,
        },
      ...
      .setFooter({ text: "معرّف المجموعة: DEMO-123" })
      .setTimestamp();

    const joinBtn = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`demo:join_new`)
        .setLabel("انضمام")
        .setEmoji("🎮")
    ...
    await channel.send({
      content: "✨ **التحديث الجديد لتنسيق المجموعات:**",
      embeds: [embed],
      components: [joinBtn]
    });

    console.log("New design demo message sent to Discord.");
    process.exit(0);
  });
}
